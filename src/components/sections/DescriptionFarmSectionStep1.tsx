...
const DescriptionFarmSectionStep1 = ({ onNext }: { onNext: () => void }) => {
  // Aquí va el contenido del paso 1 (antes DescripcionGranjaSection)
  // Puedes copiar el contenido de DescripcionGranjaSection aquí y ajustar el nombre del componente

  const [form, setForm] = useState(initialForm);
  const [farmDetailsExists, setFarmDetailsExists] = useState<string | null>(null);

  // Supón que obtienes el id y token de algún lado
  const token = useUserStore(state => state.token);
  const farmDetailsId = useFarmFormStore(state => state.formData.id);
